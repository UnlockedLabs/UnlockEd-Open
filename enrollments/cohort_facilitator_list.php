...
if ($_GET) {
    // get GET data
    $cohort_ids = isset($_GET['cohort_enrollments']) ? $_GET['cohort_enrollments'] : die('ERROR: missing COHORT ENROLLMENTS.');
    $cohorts = explode(",", $cohort_ids);

    foreach ($cohorts as $cohortId) {
        $cohort->id = $cohortId;
        $stmt = $cohort->readFacilitatorForCohort();

        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            extract($row); // cohort_name, facilitator_id, username, admin_id, course_id, cat_id
            $facilitator_name = ucfirst($username);
            $email_link = "";
            if($_SESSION['current_site_settings']['email_enabled'] == 'true'){
                $email_link = "<a href='./lc_email/lc_compose.php?recipient_id={$facilitator_id}' class='dropdown-item'><i class='icon-mail5'></i> Send message</a>";
            }
            echo <<<_FACILITATORHEAD
            <li class="media nav-item usertag">
                <a href="#" class="mr-3 position-relative" data-facilitator_id="{$facilitator_id}" data-admin_id="{$admin_id}">
                ...
                <div class="media-body align-self-center" data-facilitator_id="{$facilitator_id}" data-admin_id="{$admin_id}">
                    {$facilitator_name}
                </div>
                <div class="ml-3 align-self-center">
                    <div class="dropdown">
                        <a href="#" class="dropdown-toggle caret-0 stdnt-ham" data-toggle="dropdown"><i class="icon-more2"></i></a>
                        <div class="dropdown-menu dropdown-menu-right" data-facilitator_id="{$facilitator_id}" data-course_id="{$course_id}" data-cat_id="{$cat_id}">
                            {$email_link}
                        </div>
                    </div>
                </div>
