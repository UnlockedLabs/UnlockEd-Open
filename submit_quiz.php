...
include_once dirname(__FILE__).'/objects/submission.php';
...
if ($_POST) {
    ...
    for ($i = 1; $i <= $question_count; $i++) {
        ...
        eval('$response = isset($_POST["question_{$i}_response"]) ? $_POST["question_{$i}_response"] : die("ERROR: missing QUESTION {$i} RESPONSE");');
        ...
        eval('$question_id[] = isset($_POST["question_id_{$i}"]) ? $_POST["question_id_{$i}"] : die("ERROR: missing QUESTION ID {$i}");');
    ...
    while ($row_submission = $stmt->fetch(\PDO::FETCH_ASSOC)){
        ...
        $att_grades = implode(",", $attempt_grade);
    }
...
INCORRECTQUESTIONS;
    ...
    echo <<<RECOMMENDEDLESSONS
                    ...
                    <div class="row">
                        <ul>
                            <!-- bulleted list of concepts here -->
                        </ul>
                    </div>
RECOMMENDEDLESSONS;
    }
    ...
    echo <<<QUIZRESULTSTAIL
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <input type='hidden' name='submissions_info' value='{$quiz->quiz_name}' data-attempt_numbers='{$att_numbers}' data-attempt_grades='{$att_grades}'>
...
/*
  @todo
  submit_quiz.php
  Decide if the following code is needed and either finish it or remove it
 */
?>
...
var $scores = $("input[name='submissions_info']").data('attempt_grades');
