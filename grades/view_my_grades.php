...
if ($_GET) {
    // get GET data
    $course_id = $course->id = isset($_GET['course_id']) ? $_GET['course_id'] : die('ERROR: missing COURSE ID.');
    ...
    if (count($quizIds) == 0) {
        ...
        // read all particular students submissions for particular course id
        $stmt = $submission->readCQOsForStudentByCourse($course_id, $keptGrade);
        $num = $stmt->rowCount();
        ...
        echo <<<_QUIZZES
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title text-center">My Grades for {$course_name}</h5>
                    <!--
